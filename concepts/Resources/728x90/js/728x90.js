window.onload = function() {
  ...
  matrix.forEach(function(point, i) {
    ...
    // el.className = 'words';
    if (i < introText.length) el.innerHTML = '<i><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 195.1 20.7"><text transform="translate(0 14.417)" fill="currentColor" font-family="Arial" font-weight="bold" font-size="16.805">How do I move forward?</text></svg></i>';
    el.style.left = cX + 'px';
    el.style.top = cY + 'px';
  ...
  nodes.forEach(function (el, i, arr) {
    if (i < introText.length) {
      TweenLite.set(el, { x: 430, y: 73, opacity: 0, scale: 4.22, delay: 0 }, 0);
  ...
  TweenLite.to([bgCanvas, maskCanvas], 2, { x: -100, y: 19, transformOrigin: "50% 50%", force3D: true, rotationZ: "0.1deg", translateZ: -100, scale: .344, ease: Power2.easeOut, delay: 12.4 }, 0);
  TweenLite.to(container, 2, { x: -100, y: 19, transformOrigin: "50% 50%", force3D: true, rotationZ: "0.1deg", translateZ: -100, scale: .19, ease: Power2.easeOut, delay: 12.4 }, 0);
  ...
  TweenLite.to(text3, 1, { opacity: 1, x: 0, ease: Power2.easeOut, delay: 13.2 }, 0);
